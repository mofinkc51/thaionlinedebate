...
function AdminManageUserRow({ user, onInspect, onStatusChange }) {
  ...
  const handleStatusChange = async (e) => {
    ...
    try {
      ...
      onStatusChange(user_id, newStatus);
    } catch (error) {
  ...
  return (
    <tr className="admin-manage-user-row">
      ...
      <td className="admin-manage-user-row-status">
        <select
          id="drop-down-user-status"
          value={status === "active" ? "ปกติ" : "บัญชีถูกระงับ"}
          style={{
            backgroundColor: status === "active" ? "#EBF9F1" : "#FBE7E8",
            color: status === "active" ? "#1F9254" : "#A30D11",
          }}
      ...
      <td className="admin-manage-user-row-action">
        <div>
          <button onClick={() => onInspect(user_id)}>
